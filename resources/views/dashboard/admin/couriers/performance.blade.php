<x-modal-custom id="performance-modal-{{ $courier->id }}" title="Performa Pesanan - {{ $courier->name }}" size="4xl">
    ...
    <div id="performance-loader-{{ $courier->id }}" class="py-10 text-center">
        <svg class="inline w-8 h-8 text-gray-200 animate-spin dark:text-gray-600 fill-brand-deep" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
            {{-- SVG path --}}
        </svg>
        <p class="mt-2 text-sm text-gray-500 dark:text-gray-400">Memuat data performa...</p>
    </div>
        ...
        <div class="flex flex-wrap items-center justify-between gap-4 mb-2">
            ...
            <div class="flex items-center gap-4">
                <div class="flex items-center">
                    <span class="w-3 h-3 mr-2 bg-slate-500 rounded-full"></span>
                ...
                <div class="relative">
                    ...
                    <div id="chartFilterDropdown-{{ $courier->id }}" class="absolute right-0 z-20 hidden w-40 mt-2 bg-white border border-gray-200 rounded-lg shadow-lg dark:bg-slate-700 js-dropdown-menu dark:border-slate-600">
                        <a href="#" class="block px-4 py-2 text-sm font-semibold text-gray-700 rounded-t-lg js-performance-filter dark:text-gray-200 hover:bg-brand-light dark:hover:bg-slate-600" data-courier-id="{{ $courier->id }}" data-filter="last_7_days">7 Hari</a>
                        <a href="#" class="block px-4 py-2 text-sm font-semibold text-gray-700 js-performance-filter dark:text-gray-200 hover:bg-brand-light dark:hover:bg-slate-600" data-courier-id="{{ $courier->id }}" data-filter="daily">Harian</a>
                        <a href="#" class="block px-4 py-2 text-sm font-semibold text-gray-700 js-performance-filter dark:text-gray-200 hover:bg-brand-light dark:hover:bg-slate-600" data-courier-id="{{ $courier->id }}" data-filter="weekly">Mingguan</a>
                        <a href="#" class="block px-4 py-2 text-sm font-semibold text-gray-700 rounded-b-lg js-performance-filter dark:text-gray-200 hover:bg-brand-light dark:hover:bg-slate-600" data-courier-id="{{ $courier->id }}" data-filter="monthly">Bulanan</a>
                    </div>
                </div>
            </div>
        </div>
